<template>
  <v-row class="mx-1 mx-sm-4 mx-md-0" justify="center">
    ...
    <v-card width="100%">
      <v-row class="mx-1">
        <v-col class="pa-1" cols="4" md="3" lg="2">
          <v-img v-if="image"
            contain
            height="100%"
            aspect-ratio="1.7778"
        ...
        <v-col class="ma-1">
          ...
          <p>
                ...
                <v-card-text>
                  ...
                  <div style="margin-left: 20px;">
                    <div v-for="(entry, i) in bibtexContent" :key="i" class="d-flex" style>
                       <span>{{entry[0].toLowerCase()}}:&nbsp;</span><span>{{entry[1]}}</span>
                    </div>
                  </div>
                  <div>}</div>
                </v-card-text>
            ...
            <v-btn v-if="pub.DOI" :href="'https://doi.org/' + pub.DOI" class="ma-1" target="_blank" small>DOI</v-btn>
            <v-btn v-if="pub.URL" :href="pub.URL" class="ma-1" target="_blank" small color="blue darken-3">
                <v-icon class="mr-1">mdi-file-pdf</v-icon>
                PDF
            </v-btn>
            <v-btn v-if="youtube" :href="youtube" class="ma-1" target="_blank" small color="red darken-3">
...
export default {
  ...
  computed: {
    ...
    bibtexContent: function() {
      ...
      let filtered = entries.filter(d => { return !d[0].startsWith('BIBTEX') });
